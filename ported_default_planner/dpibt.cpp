...
int get_gp_h(TrajLNS& lns, int ai, int target){
    ...
    if (!lns.traj_dists.empty() && !lns.traj_dists[ai].empty())
        ...
        min_heuristic = d_manhattanDistance(target,lns.tasks.at(ai),lns.env);
...
bool causalPIBT(int curr_id, int higher_id,std::vector<State>& prev_states,
	...
	std::vector<dPIBT_C> successors;
	d_getNeighborLocs(&(lns.neighbors),neighbors,prev_loc);
	for (auto& neighbor: neighbors){

		assert(d_validateMove(prev_loc, neighbor, lns.env));
    ...
    for (auto& next: successors){
		if(occupied[next.location])
			continue;
		assert(d_validateMove(prev_loc, next.location, lns.env));
