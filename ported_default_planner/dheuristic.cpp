...
Neighbors d_global_neighbors;



void d_init_neighbor(SharedEnvironment* env){
	d_global_neighbors.resize(env->rows * env->cols);
	for (int row=0; row<env->rows; row++){
		for (int col=0; col<env->cols; col++){
			int loc = row*env->cols+col;
			if (env->map[loc]==0){
				if (row>0 && env->map[loc-env->cols]==0){
					d_global_neighbors[loc].push_back(loc-env->cols);
				}
				if (row<env->rows-1 && env->map[loc+env->cols]==0){
					d_global_neighbors[loc].push_back(loc+env->cols);
				}
				if (col>0 && env->map[loc-1]==0){
					d_global_neighbors[loc].push_back(loc-1);
				}
...
int get_heuristic(HeuristicTable& ht, SharedEnvironment* env, int source, Neighbors* ns){
		...
		while (!ht.open.empty())
		{
			dHNode curr = ht.open.front();
			ht.open.pop_front();

			
			d_getNeighborLocs(ns,neighbors,curr.location);
...
int get_h(SharedEnvironment* env, int source, int target){
	...
	return get_heuristic(global_heuristictable.at(target), env, source, &d_global_neighbors);
}
...
{	
	...
	while (!dp.open.empty())
	{
		...
		d_getNeighborLocs(ns,neighbors,curr.id);
