...
namespace CustomAlgo{
     ...
     */
    void PIBTTrajSolver::initialize(int preprocess_time_limit, SharedEnvironment* env){
            ...
            decided.resize(env->num_of_agents,dDCR({-1,dDONE::dDONE}));
            ...
            new (&trajLNS) TrajLNS(env, global_heuristictable, d_global_neighbors);
            trajLNS.init_mem();
     ...
     */
    void PIBTTrajSolver::plan(int time_limit,vector<Action> & actions, SharedEnvironment* env){
        cout << "Mulai ui" << std::endl;
        ...
        for(int i=0; i<env->num_of_agents; i++)
        {
            ...
            if (decided[i].loc == -1){
                ...
                assert(decided[i].state == dDONE::dDONE);
            }
            if (prev_states[i].location == decided[i].loc){
                decided[i].state = dDONE::dDONE;
            }
            if (decided[i].state == dDONE::dNOT_DONE){
        ...
        for (int id : ids){
            ...
            if (next_states.at(id).location >=0){
                decided.at(id) = dDCR({next_states.at(id).location,dDONE::dNOT_DONE});
            }

            // post process the targeted next location to turning or moving actions
            actions.at(id) = d_getAction(prev_states.at(id),decided.at(id).loc, env);
            checked.at(id) = false;
