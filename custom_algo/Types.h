...
namespace CustomAlgo{
	...
	struct HPA_H {
		...
		struct AbstractGraph{
			...
			std::map<std::pair<int,int>, int> intra;	//Antar Gate dalam cluster...
			std::map<std::pair<int,int>, int> inter;	//Antar Gate penghubung cluster..
		...
		//Test, untuk inter menggunakan gate..
		std::map<int, std::vector<int>> inter_cache;

		//Test inter hanya dengan cluster
		std::vector<std::unordered_map<int,int>> cluster_adj;
		std::unordered_map<int, std::vector<int>> cluster_cache;
