...
//         // Lazy compute IntraHT for dest if not yet done
...
//         // Same cluster — use IntraHT directly
//         if (c_src == c_dest) {
...
//         // Lazy compute cluster_cache for c_src
//         if (!env->hpa_h.cluster_cache.count(c_src)) compute_cluster_from(env, c_src);

//         int cluster_cost = env->hpa_h.cluster_cache[c_src][c_dest];
//         if (cluster_cost >= INTERVAL_MAX) return INTERVAL_MAX;

//         // FASE 1: min cost src -> any gate in c_src
//         int src_to_gate = INTERVAL_MAX;
//         for (int g_s : env->hpa_h.Gates[c_src]) {
//             int g_s_local = env->hpa_h.global_to_local[g_s];
//             if (env->hpa_h.IntraHT[c_src].find(g_s_local) == env->hpa_h.IntraHT[c_src].end()) {
//                 auto ht = build_IntraHT(env, c_src, g_s);
//                 env->hpa_h.IntraHT[c_src][g_s_local] = std::vector<std::array<int,4>>(ht.begin(), ht.end());
//             }
//             int cost = env->hpa_h.IntraHT[c_src][g_s_local][src_local][src_orient];
//             src_to_gate = std::min(src_to_gate, cost);
//         }
//         if (src_to_gate >= INTERVAL_MAX) return INTERVAL_MAX;

//         // FASE 3: min cost any gate in c_dest -> dest
//         int gate_to_dest = INTERVAL_MAX;
//         for (int g_d : env->hpa_h.Gates[c_dest]) {
//             int g_d_local = env->hpa_h.global_to_local[g_d];
//             if (env->hpa_h.IntraHT[c_dest].find(dest_local) == env->hpa_h.IntraHT[c_dest].end()) continue;
//             for (int o = 0; o < 4; o++) {
//                 gate_to_dest = std::min(gate_to_dest,
//                     env->hpa_h.IntraHT[c_dest][dest_local][g_d_local][o]);
//             }
//         }
//         if (gate_to_dest >= INTERVAL_MAX) return INTERVAL_MAX;

//         return src_to_gate + cluster_cost + gate_to_dest;
//     }
    int query_heuristic(SharedEnvironment* env, int src, int src_orient, int dest) {
